...
def get_text_messages(message):
    ...
    if message.text == "Привет" or message.text == "привет":
        ...
        # Выводим все кнопки сразу и пишем сообщение о выборе
        bot.send_message(message.from_user.id, text='Выбери свой знак зодиака', reply_markup=keyboard)
    elif message.text == "/help":
        bot.send_message(message.from_user.id, "Напиши Привет")
    elif message.text == "/start":
        bot.send_message(message.from_user.id, "Котик-гороскопщик приветствует тебя, мой любопытный друг!")
    else:
        bot.send_message(message.from_user.id, "Я тебя не понимаю. Зови помощь")
...
def callback_worker(call):
    ...
    if call.data == "zodiac":
        ...
        # Отправляем текст в ТГ
        bot.send_message(call.message.chat.id, msg)

# Запускаем постоянный опрос бота в ТГ
